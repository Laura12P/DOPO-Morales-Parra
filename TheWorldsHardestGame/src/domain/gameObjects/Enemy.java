package domain.gameObjects;

import java.awt.Color;
import java.awt.Graphics;

/* La clase Enemy es una subclase abstracta de Element, que al colisionar con un player, aumenta el contador de muertes del
 * mismo y lo hace respawnear.
 * 
 * @author Laura Juliana Parra Velandia y Daniel Santiago Morales Perdomo
 */

public abstract class Enemy extends Element {
	
	/*Contructor de la clase Enemy
	 * 
	 * @param positionX Numero entero positivo que define la posicion en el eje X de un enemigo.
	 * @param positionY Numero entero positivo que define la posicion en el eje Y de un enemigo.
	 * @param width Numero entero positivo que define el ancho de un enemigo.
	 * @param height Numero entero positivo que define la altura de un enemigo.
	 * @param speed Numero entero positivo que define la velocidad base de un enemigo.
	 * @param color Color actual de un enemigo.
	 */
	public Enemy(double positionX, double positionY, double width, double height, double speed, Color color) {
		super(positionX, positionY, width, height, speed, color);
	}
	
	@Override
	public void draw(Graphics g) {
	    g.setColor(color);
	    g.fillOval((int) positionX, (int) positionY, (int) width, (int) height);
	    g.setColor(Color.BLACK);
	    g.drawOval((int) positionX, (int) positionY, (int) width, (int) height);
	}
	
	public abstract void move(int boardWidth, int boardHeight);

}